...
\begin{columns}[T,totalwidth=\textwidth]
  \column{0.55\textwidth}
  \vspace{-1mm}
  \begin{itemize}\scriptsize\setlength{\itemsep}{2.5mm}
    \item Para los grupos que \alert{no cumplen} [F.U. $>1{,}0$] se planteó una idealización:
          escalar los perfiles excedidos a secciones \alert{TUBEST mayores} conservando la
          tipología [p. ej. TB $550\times200\times4$-$5\times3 \to$ TB $550\times225\times6\times3$].
    \item Resultado: el F.U. disminuye, pero \alert{no lo suficiente} --- varios elementos
          siguen sin cumplir, gobernados por $D+L_r$ y por el sismo amplificado.
    \item Causa: el estado límite que gobierna es el \alert{pandeo} [lateral-torsional en
          vigas, flexo-compresión en columnas], que depende de la longitud no arriostrada
          $L_b \sim$ luz del vano. Como $L_b$ \alert{no cambia} al engrosar la sección, la
          mejora se \alert{satura}: reforzar solo por sección es ineficiente para los
          grandes vanos de la Nave~F.
  \end{itemize}
  \column{0.43\textwidth}
  \centering
  {\scriptsize\textbf{F.U. modelo idealizado:}}\\[2mm]
  \fittab{\tablasize
  \begin{tabular}{@{}lcc@{}}
    \toprule
    Grupo & F.U. & Estado \\
    \midrule
    Vigas 1    & 1,62 & \textcolor{red}{\textbf{NO CUMPLE}} \\
    Columnas 1 & 1,42 & \textcolor{red}{\textbf{NO CUMPLE}} \\
    Columnas 3 & 1,09 & \textcolor{red}{\textbf{NO CUMPLE}} \\
    \bottomrule
  \end{tabular}}\\[2mm]
  {\scriptsize Aumentar la sección reduce el F.U. cada vez menos
  $\Rightarrow$ solución insuficiente.}
\end{columns}
\end{frame}
...
\vspace{3mm}
\begin{itemize}\small\setlength{\itemsep}{4mm}
  \item Como \alert{ninguna} idealización basada solo en aumentar la sección logra el
        cumplimiento, un análisis de \alert{peso y costo} de la adecuación \alert{no es
        representativo} en esta etapa y queda fuera del alcance: comparar \$/kg de acero
        frente a una solución que aún no cumple no entrega una conclusión económica válida.
  \item La vía recomendada \alert{no es seguir aumentando secciones}, sino \alert{reducir las
        longitudes efectivas}: incorporar \alert{arriostramientos o apoyos intermedios} que
        disminuyan $L_b$ y la esbeltez. Al acortar los vanos, la capacidad al pandeo aumenta
        de forma directa y se recupera el cumplimiento con perfiles menores.
  \item Definida esa reconfiguración geométrica, sí sería posible cuantificar el \alert{peso
        de acero adicional y el costo} [\$/kg] respecto de la estructura original.
\end{itemize}
\end{frame}
...
\begin{itemize}\scriptsize\setlength{\itemsep}{1.6mm}
  ...
  \item \alert{Adecuación}: aumentar solo las secciones TUBEST \alert{no logra} el
        cumplimiento, porque gobierna el \alert{pandeo} asociado a los grandes vanos. Se
        recomienda un \alert{replanteo geométrico} [arriostramientos / apoyos intermedios];
        el análisis económico queda condicionado a esa reconfiguración.
  \item \alert{Síntesis}: la Nave~F, satisfactoria bajo las normas con que fue concebida,
        \alert{no cumple} las exigencias vigentes de 2025 --- principalmente por el incremento
        sísmico. Su adecuación es factible, pero exige \alert{repensar la configuración
        estructural} más que sobredimensionar los perfiles.
\end{itemize}
\end{frame}
